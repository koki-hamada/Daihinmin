...
#define DEFAULT_NAME		"Ganesa"	//デフォルトのクライアント名 文字列で指定

extern const int g_logging;

/* 静的関数の関数プロトタイプ宣言 */
static int refreshTable(int table_val[8][15], int idx);
static int sendTable(int table_val[8][15], int idx);
static int openSocket(const char ip_addr[], uint16_t portnum_data, int idx);
static int sendProfile(const char user[15], int idx);
...
int entryToGame(int idx) {
	...
	if ((openSocket(server_name, port, idx)) == -1) {
		printf("failed to open socket to server[%s:%d].\n", server_name, port);
		exit(1);
	}
	if (g_logging == 1) {
		printf("connecting to server was finished successfully[%s:%d].\n", server_name, port);
	}

	sendProfile(user_name, idx);     //クライアントの情報を送信

	if (g_logging == 1) {
		printf("send profile .\n");
	}
	//自身のプレイヤー番号をサーバからもらう
	if (read(g_sockfd[idx], &my_playernum, sizeof(my_playernum)) > 0) {
...
int sendCards(int cards[8][15], int idx) {
	...
	if ((sendTable(cards, idx)) == -1) { //カードを送信し 失敗時はメッセージを表示
		printf("failed to send card sending table. \n");
		exit(1);
	}

	//accept_flagをサーバから受け取る
	if ((read(g_sockfd[idx], &accept_flag, sizeof(accept_flag))) < 0) {
...
int beGameEnd(int idx) {
	...
	//ゲーム終了のフラグを受け取る
	if ((read(g_sockfd[idx], &one_gameend_flag, sizeof(one_gameend_flag))) < 0) {
		//受信失敗時 メッセージを表示し停止
...
static int refreshTable(int table_val[8][15], int idx) {
	...
	if ((g_buf_len = read(g_sockfd[idx], net_table, 480)) > 0) {
		int i, j;
...
static int sendTable(int table_val[8][15], int idx) {
	...
	//変換したテーブルを送信
	if ((g_buf_len = write(g_sockfd[idx], net_table, 480)) > 0) {
		return (0);
	} else {
		return (-1);
	}
...
static int openSocket(const char addr[], uint16_t port_num, int idx) {
	//ソケットの生成
	if ((g_sockfd[idx] = socket(PF_INET, SOCK_STREAM, 0)) < 0) {
		return (-1);
	}
	/* ポートとアドレスの設定 */
	...
	/* サーバにコネクトする */
	if (connect(g_sockfd[idx], (struct sockaddr *) &g_client_addr, sizeof(g_client_addr)) == 0) {
		return 0;
	}
	return -1;
}
...
static int sendProfile(const char user[15], int idx) {
	...
	for (i = 0; i < 15; i++)
		profile[1][i] = (int) user[i];     //制作者名をテーブルに格納

	//送信
	if (sendTable(profile, idx) == -1) {                       //失敗したらエラーを出力し停止
