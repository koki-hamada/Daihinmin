/*
 softmaxClassifier.hpp
 Katsuki Ohto
 */

#ifndef UTIL_SOFTMAX_CLASSIFIER_HPP_
#define UTIL_SOFTMAX_CLASSIFIER_HPP_
...
public:
    ...
    double limit(int i, int st = 0)const{
        ...
        // 特徴量の値の分散が大きいとき
        // にはパラメータの値を小さめに制限
        return PARAM_SUM_VALUE_MAX / sqrt(sqrt((double)N_PARAMS_)) / sqrt(max(all_phase_var(i, st), 1.0));
    }
    ...
    void updateParams(int ph = 0, int st = 0){
        ...
        if(sparseUpdate){ // 勾配を使用した特徴1つずつで更新する場合
            ...
            pclassifier_->lock();
            const double weight = sqrt((double)batch_);
...
#endif // UTIL_SOFTMAX_CLASSIFIER_HPP_
